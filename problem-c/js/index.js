...
function huskiesLost(game) {
  if (game.opponent_score > game.home_score && game.opponent == 'UW') {
    return false;
  } else if (game.home_score > game.opponent_score) {
    return false;
  } else {
    return true;
  }
...
let lostgames = lost.map(extractOpponent);
...
lost.forEach(function(element) {
  console.log(element.opponent + " at " + element.home + ", " + element.opponent_score + " to " + element.home_score);
});
...
let fumble = huskyGames2016.filter(function(item) {
...
function makeCombinedFilter(callback1, callback2) {
  function combinedFilter(game) {
    let result = callback1(game) && callback2(game);
    return result;
  }
...
let lostandfumbled = huskyGames2016.filter(fumbledAndLostFilter);
