...
// The output tells us that the function adds four based on the type of data inputted.
...
function compoundInterest(balance, interest, years) {
  let compound = balance * Math.exp(interest * years);
  return compound;
}
...
function isFlush(cards) {
  let suitcheck = cards[0].suit;
  let tf = false;
  for (let i = 1; i < cards.length; i++) {
    if (cards[i].suit == suitcheck) {
      tf = true;
    } else {
      return false;
    }
